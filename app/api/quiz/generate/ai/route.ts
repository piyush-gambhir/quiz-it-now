  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (formattedData && !('error' in formattedData)) {
        logger.info('Successfully generated and formatted LLM response');
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { userId, input, inputType, numberOfQuestions, difficulty, model } =
      data;
    ...
    if (inputType === 'text') {
      ...
      if (inputUrl.includes('youtube.com/watch?v=')) {
        ...
        try {
          ...
          get_youtube_transcript_url.searchParams.set('url', inputUrl);
          const response = await fetch(get_youtube_transcript_url).then((res) =>
            res.json(),
          );
          if (!response.success) {
          ...
          return NextResponse.json(
            {
              ...
              error: {
                message:
                  error.message || 'Failed to fetch the YouTube transcript.',
                type: 'API_ERROR',
